...
export class HomeComponent implements OnInit, AfterViewChecked {
  ...
  ngOnInit(): void {
    this.dataToBeNavigated = history.state;
    if (this.dataToBeNavigated && this.dataToBeNavigated.id) {
  ...
  getAccessToken(): void {
    this.authenticationService.getToken().subscribe(
      (res) => {
        this.loadingPage(this.dataToBeNavigated);
      },
      (error) => {
        console.log(error);
      }
  ...
  loadingPage(dataToBeNavigated): void {
    // const req = {
    //   active: {
    ...
    //   sort: [['id,asc']]
    // };
    this.employeeService.find(this.dataToBeNavigated.id).subscribe(
